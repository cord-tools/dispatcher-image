...
RUN yum -y --setopt=tsflags=nodocs install epel-release; \
    ...
    yum --enablerepo=epel -y --setopt=tsflags=nodocs install inotify-tools httpd; \
    yum clean all
...
RUN echo "Installing dispatcher version ${DISPATCHER_VERSION}"; \
    curl -L "http://download.macromedia.com/dispatcher/download/dispatcher-apache2.4-linux-x86_64-ssl-${DISPATCHER_VERSION}.tar.gz" -O; \
    mkdir dispatcher; \
